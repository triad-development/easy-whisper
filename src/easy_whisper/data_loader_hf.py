    ...
    try:
        ...
        for split_name, split_dataset in dataset.items():
            ...
            if text_column is None:
                common_text_columns = ['text', 'sentence', 'transcription', 'transcript']
            ...
            if column_mapping:
                split_dataset = split_dataset.rename_columns(
                    {old: new for old, new in column_mapping.items() if old in split_dataset.column_names}
                )
            ...
            for split_name in dataset:
                ...
                if 'audio' in dataset[split_name].column_names:
                    ...
                    if not isinstance(audio_feature, Audio):
                        logger.info(f"Converting '{split_name}' audio column to Audio feature")
                        dataset[split_name] = dataset[split_name].cast_column('audio', Audio(sampling_rate=sampling_rate))
                    # If it's already Audio but with different sampling rate
                    elif audio_feature.sampling_rate != sampling_rate:
                        logger.info(f"Resampling '{split_name}' audio from {audio_feature.sampling_rate}Hz to {sampling_rate}Hz")
                        dataset[split_name] = dataset[split_name].cast_column('audio', Audio(sampling_rate=sampling_rate))
        
        # Ensure required columns exist
        for split_name, split_dataset in dataset.items():
            if 'audio' not in split_dataset.column_names:
                raise ValueError(f"Split '{split_name}' does not contain an 'audio' column after processing")
            if 'sentence' not in split_dataset.column_names:
                raise ValueError(f"Split '{split_name}' does not contain a 'sentence' column after processing")
        
        # Keep only the necessary columns
        for split_name in dataset:
            if set(dataset[split_name].column_names) != {'audio', 'sentence'}:
                dataset[split_name] = dataset[split_name].select_columns(['audio', 'sentence'])

...
def compute_metrics(pred, tokenizer):
	...
	try:
		...
		logger.warning(
			f"Error in batch_decode: {e}. Falling back to individual decoding."
		)
...
def train_model(
	...
	if push_to_hub:
		...
		if not ensure_hf_login(force_login=force_hf_login):
			logger.warning(
				"Failed to log in to Hugging Face Hub. Disabling push_to_hub."
			)
			push_to_hub = False
	...
	try:
		processor = WhisperProcessor.from_pretrained(
			model_name_or_path, language=language, task=task, token=use_auth_token
		)
	...
	for split in dataset:
		for col in required_columns:
			if col not in dataset[split].column_names:
				raise ValueError(
					f"Column '{col}' not found in dataset['{split}']. Required columns: {required_columns}"
				)

		# ensure audio column is in the Audio format
		if not isinstance(dataset[split].features["audio"], Audio):
			raise ValueError(
				f"Column 'audio' in dataset['{split}'] must be in Audio format"
			)
	...
	if safe_multiprocessing and platform.system() == "Windows":
		...
		logger.info(
			"Running on Windows - Setting num_proc=1 to avoid multiprocessing issues"
		)
	...
	for split in dataset:
		...
		if max_effective_workers < effective_num_workers:
			logger.info(
				f"Reducing num_proc to {max_effective_workers} for dataset of size {split_size}"
			)
		...
		try:
			...
			if "An attempt has been made to start a new process" in str(e):
				logger.warning(
					"Multiprocessing error detected. Falling back to single process."
				)
...
def load_finetuned_model(
	...
	is_peft_model = os.path.exists(
		os.path.join(model_path_or_id, "adapter_config.json")
	)
	...
	if is_peft_model:
		...
		processor = WhisperProcessor.from_pretrained(
			base_model_path, language=language, task=task
		)
	else:
		...
		processor = WhisperProcessor.from_pretrained(
			model_path_or_id, language=language, task=task
		)
